...
SCRIPT_PATH = PROJECT_ROOT / "blender" / "scripts" / "gerar_faca_boning.py"
BLEND_PATH = PROJECT_ROOT / "blender" / "cenas" / "faca" / "faca_boning_dataset.blend"
...
def create_scene():
    ...
    background_material = create_material("Material_Fundo", (0.004, 0.004, 0.005), 1.0, 0.0)
...
def randomize_scene(refs):
    ...
    refs["key"].data.energy = random.uniform(300, 410)
    ...
    refs["fill"].data.energy = random.uniform(70, 125)
    ...
    refs["rim"].data.energy = random.uniform(20, 46)
...
def main():
    ...
    for index in range(1, TOTAL_IMAGES + 1):
        ...
        if index % SAVE_BLEND_EVERY == 0 or index == TOTAL_IMAGES:
            save_blend()

        print("PROGRESSO: {}/{}".format(index, TOTAL_IMAGES))
