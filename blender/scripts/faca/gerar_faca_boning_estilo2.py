...
SCRIPT_PATH = PROJECT_ROOT / "blender" / "scripts" / "gerar_faca_boning_estilo2.py"
BLEND_PATH = PROJECT_ROOT / "blender" / "cenas" / "faca" / "faca_boning_estilo2.blend"
...
def create_blade_material():
    color = random.choice(BLADE_COLORS)
    material = bpy.data.materials.new(name="Material_Lamina_Estilo2")
    material.use_nodes = True
    nodes = material.node_tree.nodes
    ...
    principled.inputs["Base Color"].default_value = (color[0], color[1], color[2], 1.0)
    principled.inputs["Metallic"].default_value = random.uniform(0.82, 0.95)
    principled.inputs["Roughness"].default_value = random.uniform(0.16, 0.24)
    principled.inputs["Specular"].default_value = 0.55

    noise = nodes.new(type="ShaderNodeTexNoise")
    noise.location = (-250, -80)
    noise.inputs["Scale"].default_value = random.uniform(120.0, 220.0)
    noise.inputs["Detail"].default_value = 2.0
    noise.inputs["Roughness"].default_value = 0.35

    bump = nodes.new(type="ShaderNodeBump")
    bump.location = (-10, -80)
    bump.inputs["Strength"].default_value = random.uniform(0.008, 0.020)
...
def create_handle_material():
    color = random.choice(HANDLE_COLORS)
    material = bpy.data.materials.new(name="Material_Cabo_Estilo2")
    material.use_nodes = True
    nodes = material.node_tree.nodes
    links = material.node_tree.links
    ...
    principled.inputs["Roughness"].default_value = random.uniform(0.52, 0.70)
    principled.inputs["Specular"].default_value = 0.35

    noise = nodes.new(type="ShaderNodeTexNoise")
    noise.location = (-250, -80)
    noise.inputs["Scale"].default_value = random.uniform(35.0, 60.0)
    noise.inputs["Detail"].default_value = 4.0
    noise.inputs["Roughness"].default_value = 0.55

    bump = nodes.new(type="ShaderNodeBump")
    bump.location = (-10, -80)
    bump.inputs["Strength"].default_value = random.uniform(0.030, 0.060)
...
def create_scene():
    ...
    table_material = create_principled_material("Material_Mesa_Estilo2", WOOD_COLORS[0], 0.58, 0.0)
    ...
    background.scale = (1.9, 1.45, 1.0)
    background_material = create_principled_material("Material_Fundo_Estilo2", (0.005, 0.005, 0.006), 1.0, 0.0)
    background.data.materials.append(background_material)

    bpy.ops.object.light_add(type="AREA", location=(0.50, -0.48, 1.02))
    key = bpy.context.active_object
    key.name = "Luz_Principal"
    key.data.energy = 340
    key.data.size = 0.70

    bpy.ops.object.light_add(type="AREA", location=(-0.50, -0.08, 0.82))
    fill = bpy.context.active_object
    fill.name = "Luz_Preenchimento"
    fill.data.energy = 88
    fill.data.size = 0.90

    bpy.ops.object.light_add(type="POINT", location=(0.0, 0.26, 0.94))
...
def randomize_scene(refs):
    ...
    refs["key"].data.energy = random.uniform(300, 390)
    refs["key"].location = Vector((
        random.uniform(0.36, 0.62),
        random.uniform(-0.60, -0.34),
        random.uniform(0.92, 1.10)
    ))

    refs["fill"].data.energy = random.uniform(72, 118)
    refs["fill"].location = Vector((
        random.uniform(-0.62, -0.36),
        random.uniform(-0.22, 0.04),
        random.uniform(0.72, 0.92)
    ))

    refs["rim"].data.energy = random.uniform(18, 42)
    refs["rim"].location = Vector((
        random.uniform(-0.12, 0.12),
        random.uniform(0.18, 0.34),
        random.uniform(0.84, 1.00)
    ))
    ...
    camera.data.lens = random.uniform(62, 74)

    look_at(
        camera,
        Vector((
            random.uniform(-0.030, 0.030),
            random.uniform(-0.020, 0.020),
            TABLE_TOP_Z + random.uniform(0.006, 0.024)
        ))
    )
...
def import_and_join_model():
    ...
    if not imported_meshes:
        raise RuntimeError("Nenhuma malha foi importada.")
    ...
    knife.name = "Faca_Boning_Estilo2"
...
def assign_materials(knife):
    blade_material = create_blade_material()
    handle_material = create_handle_material()
...
def refresh_knife_materials(knife):
    assign_materials(knife)
...
def bbox_is_valid(bbox):
    ...
    if longest < 0.30 or longest > 0.64:
        return False
...
def generate_one(index, refs, model):
    ...
    for attempt in range(1, MAX_ATTEMPTS + 1):
        ...
        refresh_knife_materials(model["object"])
        place_knife(model)
        bpy.context.view_layer.update()
