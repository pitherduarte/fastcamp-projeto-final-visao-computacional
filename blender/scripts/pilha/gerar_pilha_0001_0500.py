...
BLEND_SAVE_DIR = r"C:\Users\pithe\Documents\Projetos\Github\fastcamp-projeto-final-visao-computacional\blender\cenas"
BLEND_FILEPATH = os.path.join(BLEND_SAVE_DIR, "pilha_dataset_500.blend")
...
def save_blend_file():
    try:
        ...
        print(f"BLEND SALVO EM: {BLEND_FILEPATH}")
...
def create_simple_material(name, color, metallic, roughness, specular):
...
def create_area_light(name, collection, energy, size):
...
def configure_lights(key, fill, rim, center_x, center_y):
    key.location = (
...
def build_battery_template(
    ...
    copper_depth = BATTERY_LENGTH * COPPER_FRACTION
    ...
    top_cap_depth = BATTERY_LENGTH * 0.025
    ...
    nub_depth = BATTERY_LENGTH * 0.045
    ...
    bottom_depth = BATTERY_LENGTH * 0.018
...
def duplicate_hierarchy(
    ...
    if source.data is not None:
        obj_copy.data = source.data.copy()
...
def choose_count(index):
    local_index = index - START_INDEX + 1
...
def apply_pose(root, orientation, x, y):
    if orientation == "UP":
        root.location = (
            x,
            y,
            TABLE_Z + BATTERY_LENGTH * 0.51
        )
...
def place_batteries(roots, orientations):
    placed = []
    ...
    for root, orientation in zip(
        ...
        for attempt in range(800):
            ...
            for existing_x, existing_y, existing_radius in placed:
                distance = math.sqrt(
                    (x - existing_x) ** 2
                    + (y - existing_y) ** 2
                )
...
def get_yolo_bbox(root, camera, scene):
...
def main():
    ...
    print("=" * 72)
    print("PILHA | GERACAO FINAL 500 IMAGENS")
