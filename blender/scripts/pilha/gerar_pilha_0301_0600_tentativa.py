...
BLEND_FILEPATH = os.path.join(BLEND_SAVE_DIR, "pilha_dataset_0301_0600.blend")
...
def build_battery_template(
    ...
    set_hierarchy_visibility(root, False)
...
def create_battery_pool(
    ...
    for index in range(MAX_BATTERIES):
        ...
        root.name = f"BatteryPool_{index + 1:02d}"
        set_hierarchy_visibility(root, False)
        pool.append(root)
...
def configure_pool(pool, count):
    active = []

    for index, root in enumerate(pool):
        visible = index < count
        set_hierarchy_visibility(root, visible)
...
def main():
    ...
    print("=" * 72)
    print("PILHA | GERACAO OTIMIZADA DE 301 A 600")
