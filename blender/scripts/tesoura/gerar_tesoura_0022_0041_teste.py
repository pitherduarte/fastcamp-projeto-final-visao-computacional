...
BLEND_OUTPUT = os.path.join(PROJECT_ROOT, "blender", "tesoura", "tesoura_scissor_2_teste.blend")
...
def set_render_settings():
    ...
    if background:
        background.inputs[0].default_value = (0.58, 0.59, 0.61, 1.0)
...
def randomize_base(base):
    ...
    if bsdf:
        value = random.uniform(0.50, 0.62)
        bsdf.inputs["Base Color"].default_value = (
            value,
            min(value + 0.01, 0.64),
            min(value + 0.025, 0.66),
            1.0
        )
...
def randomize_world():
    ...
    if background:
        value = random.uniform(0.48, 0.62)
        ...
        background.inputs[1].default_value = random.uniform(0.12, 0.22)
...
def randomize_lights(lights):
    ...
    lights["key"].data.energy = random.uniform(480, 720)
    ...
    lights["fill"].data.energy = random.uniform(150, 260)
    ...
    lights["top"].data.energy = random.uniform(95, 185)
