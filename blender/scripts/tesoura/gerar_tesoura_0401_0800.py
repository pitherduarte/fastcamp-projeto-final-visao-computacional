...
BLEND_OUTPUT = os.path.join(PROJECT_ROOT, "blender", "tesoura", "tesoura_scissor_2_401_800_escuro.blend")
...
def set_render_settings():
    ...
    try:
        ...
        pass
    scene.view_settings.exposure = -1.90
    ...
    if background:
        background.inputs[0].default_value = (0.34, 0.35, 0.37, 1.0)
        background.inputs[1].default_value = 0.07
...
def randomize_base(base):
    ...
    if bsdf:
        value = random.uniform(0.32, 0.42)
        bsdf.inputs["Base Color"].default_value = (
            value,
            min(value + 0.01, 0.44),
            min(value + 0.025, 0.46),
            1.0
        )
        bsdf.inputs["Roughness"].default_value = random.uniform(0.80, 0.90)
...
def randomize_world():
    ...
    if background:
        value = random.uniform(0.28, 0.38)
        background.inputs[0].default_value = (
            value,
            min(value + 0.01, 0.40),
            min(value + 0.025, 0.42),
            1.0
        )
        background.inputs[1].default_value = random.uniform(0.04, 0.08)
...
def randomize_lights(lights):
    ...
    lights["key"].data.energy = random.uniform(250, 360)
    ...
    lights["fill"].data.energy = random.uniform(60, 105)
    ...
    lights["top"].data.energy = random.uniform(30, 60)
...
def randomize_pose(root, meshes, base_rotation):
    root.rotation_euler = (
        ...
        base_rotation.z + math.radians(random.uniform(-42, 42))
    )
...
def main():
    ...
    if not os.path.isfile(MODEL_PATH):
        raise FileNotFoundError("Modelo não encontrado: {}".format(MODEL_PATH))
    ...
    print("=" * 72)
    print("GERACAO DO DATASET DE TESOURA")
