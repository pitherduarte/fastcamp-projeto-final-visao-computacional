...
BLEND_PATH = os.path.join(SCENE_DIR, "tomada_americana_500.blend")

PREFIX = "tomada_americana"
COUNT = 500
CLASS_ID = 3
SEED_BASE = 7300
...
def remove_previous_files():
    ...
    print("Imagens anteriores removidas:", removed_images)
    print("Labels anteriores removidos:", removed_labels)
...
def generate_dataset():
    ...
    print("=" * 78)
    print("TOMADA AMERICANA | CLASSE YOLO 3")
    print("=" * 78)
    print("Quantidade:", COUNT)
    print("Resolução: 640 x 640")
    ...
    for index in range(1, COUNT + 1):
        ...
        if not bbox_is_valid(valid_bbox):
            raise RuntimeError(
                "Falha ao gerar caixa YOLO válida para a imagem {}".format(
                    index
                )
            )
        ...
        average = elapsed / generated
        remaining = average * (COUNT - generated)

        print(
            "[{:03d}/{:03d}] {} | caixa {:.3f} x {:.3f} | restante {}".format(
                generated,
                COUNT,
                filename,
                valid_bbox[2],
    ...
    print("Configurações de segurança usadas:", fallback_count)
